...
make_heatmap = function(X, y, panel){
  ...
  X[which(X < -2)] <- -2
  X[which(X > 2)] <- 2
  NMF::aheatmap(X, annRow = list(subtype = y))
}
```
...
| biomarker panel | biomarker signature, multimarker panel | combination of multimarker panels
...
## Apply biomarker panels to test cohorts

.pull-left[

### Diff_plsda_biomarker_panel
...
## Simulation scenerio (true error rate = 50%)
...
## The correct way of determining performance.

```{r}
right_model <- plsda(X = eset, Y = group, ncomp=1)

cv <- perf(right_model, validation = "Mfold", folds = 5, nrepeat = 5, auc=TRUE)
cv$auc
```
...
### Stability of variables in the model

* frequency of variable across folds.

---

### Esimate uncertainty of model parameter estimates
...
## Cross-validation
- [Resamplign techniques](https://hastie.su.domains/MOOC-Slides/cv_boot.pdf)
